 ...
 */
export function makeAuthMiddleware(getTokenHash: () => string | null): (request: FastifyRequest) => Promise<void> {
  return async (request) => {
    ...
    if (url.startsWith('/healthz') || url === '/' || url.startsWith('/assets/')) return;
