...
export type Capability =
  ...
  | "thinking"
  | "image-generation"
  | string; // string = rozszerzalna lista
```
...
| `spec-type`                     | speculative      | enum (none, draft-_, ngram-_)          | none                                | `--spec-type`                                                  |
...
- **Alternatywy odrzucone**:
  - _Python/FastAPI_: świetny w ekosystemie ML, ale dwa języki (TS+Py),
  ...
  - _Go_: lepszy proces management, ale brak wspólnych typów z FE +
    wolniejszy development (kompilacje, mniej ekosystemu UI).
  - _Electron/Tauri (desktop app)_: użytkownik chce przeglądarki + lokalnego
