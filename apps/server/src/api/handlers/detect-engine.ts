...
export async function detectEngineHandler(_request: FastifyRequest, reply: FastifyReply): Promise<void> {
  ...
  if (!found) {
    try {
      const which = execSync('which llama-server', { timeout: 2000, stdio: 'pipe' }).toString().trim();
