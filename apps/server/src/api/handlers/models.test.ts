...
describe('models endpoints (PLAN §14.1)', () => {
  ...
  it('POST /api/v1/models adds a model, then GET/:id, PATCH, DELETE', async () => {
    ...
    const get = await app.inject({ method: 'GET', url: `/api/v1/models/${added.id}` });
    ...
    const del = await app.inject({ method: 'DELETE', url: `/api/v1/models/${added.id}` });
  ...
  it('GET /api/v1/models/:id returns 404 for an unknown id', async () => {
    const { app } = await tempApp();
    const res = await app.inject({ method: 'GET', url: '/api/v1/models/unknown-id' });
  ...
  it('POST /api/v1/models/discover returns added/removed/total', async () => {
    const { app } = await tempApp();
    const res = await app.inject({ method: 'POST', url: '/api/v1/models/discover' });
    expect(res.statusCode).toBe(200);
    const body = res.json();
