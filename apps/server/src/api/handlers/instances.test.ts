...
async function tempApp(port?: number) {
  ...
  store.writePreset(MODEL_ID, "fast", {
    version: 1,
    name: "fast",
    port,
    params: {},
...
describe("instance endpoints (PLAN §14.1, Faza 5.4)", () => {
  ...
  it("GET /api/v1/instances lists known instances", async () => {
    ...
    try {
      ...
      expect(
        body.instances.some(
          (i: { instanceId: string }) => i.instanceId === INSTANCE,
        ),
      ).toBe(true);
    } finally {
      lifecycle.shutdown();
  ...
  it("full lifecycle over HTTP: start → running → stop → restart", async () => {
    ...
    try {
      const start = await app.inject({
        method: "POST",
        url: `/api/v1/instances/${INSTANCE}/start`,
      });
      ...
      const stop = await app.inject({
        method: "POST",
        url: `/api/v1/instances/${INSTANCE}/stop`,
      });
      ...
      const restart = await app.inject({
        method: "POST",
        url: `/api/v1/instances/${INSTANCE}/restart`,
      });
  ...
  it("POST start on an unknown instance → 404", async () => {
    ...
    try {
      const res = await app.inject({
        method: "POST",
        url: "/api/v1/instances/unknown--preset/start",
      });
  ...
  it("GET /api/v1/instances/:id/metrics → runtime metrics (Faza 6.3)", async () => {
    ...
    try {
      const res = await app.inject({
        method: "GET",
        url: `/api/v1/instances/${INSTANCE}/metrics`,
      });
      expect(res.statusCode).toBe(200);
      const body = res.json();
  ...
  it("GET /api/v1/instances/:id/logs → recent log lines (Faza 6.3)", async () => {
    ...
    try {
      const res = await app.inject({
        method: "GET",
        url: `/api/v1/instances/${INSTANCE}/logs`,
      });
      expect(res.statusCode).toBe(200);
      const body = res.json();
      expect(body.instanceId).toBe(INSTANCE);
  ...
  it("GET /api/v1/instances/:id on an unknown instance → 404 (Faza 6.3)", async () => {
    ...
    try {
      const res = await app.inject({
        method: "GET",
        url: "/api/v1/instances/unknown--preset",
      });
      expect(res.statusCode).toBe(404);
    } finally {
      lifecycle.shutdown();
