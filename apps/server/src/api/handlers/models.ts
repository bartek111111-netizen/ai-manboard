...
export function makeModelHandlers(store: ConfigStore) {
  ...
  return {
    /** GET /api/v1/models */
    listModels: async (_request: FastifyRequest, reply: FastifyReply): Promise<void> => {
      reply.send({ models: registry.list() });
    },

    /** POST /api/v1/models/discover — trigger a rescan (FM-2). */
    discover: async (_request: FastifyRequest, reply: FastifyReply): Promise<void> => {
      reply.send(registry.discover());
    },

    /** POST /api/v1/models — manual add (FM-3). */
    addModel: async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    ...
    getModel: async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    ...
    updateModel: async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
      const id = String((request.params as Record<string, string>).modelId);
    ...
    removeModel: async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
      const id = String((request.params as Record<string, string>).modelId);
      registry.remove(id);
