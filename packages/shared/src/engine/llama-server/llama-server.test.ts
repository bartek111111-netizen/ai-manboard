...
describe("LlamaServerEngine (PLAN §7.1, §10 — Faza 2)", () => {
  ...
  function fakeBinary(
    name: string,
    versionLine = "version: 9.9.9-test",
  ): string {
  ...
  it("detectCapabilities: heuristics from filename/architecture", () => {
    expect(
      engine.detectCapabilities({
        id: "m1",
        path: "/x/Qwen3.5-4B-Instruct-Q8.gguf",
        engineId: "llama-server",
      }),
  ...
  it("validate: missing model/draft file → problem; present files → ok", () => {
    ...
    expect(
      engine.validate(
        modelInfo,
        resolved({ "spec-model": "/nope/draft.gguf" }),
      ),
    ).toEqual(["spec-model: the draft model does not exist: /nope/draft.gguf"]);
    expect(
      engine.validate(
        modelInfo,
        resolved({ model: modelFile, "spec-model": modelFile }),
      ),
    ).toEqual([]);
    expect(
      engine.validate(modelInfo, resolved({ model: "/nope/missing.gguf" })),
    ).toEqual(["model: the file does not exist: /nope/missing.gguf"]);
  ...
  it("preflight: gpu-layers>0 on a non-Vulkan binary → ENGINE_BINARY_INVALID (blocks)", async () => {
    ...
    expect(result.errors.some((e) => e.code === "ENGINE_BINARY_INVALID")).toBe(
      true,
    );
    expect(result.errors.some((e) => e.message.includes("gpu-layers=0"))).toBe(
      true,
    );
  });
