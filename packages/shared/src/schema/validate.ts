 ...
 */
export function validateGlobalConfig(data: unknown, label = 'global.json'): GlobalConfig {
  ...
  if (typeof data.version !== 'number') problems.push('version: missing or not a number');

  if (!Array.isArray(data.modelDirs)) {
    problems.push('modelDirs: expected an array of absolute paths');
  } else {
    data.modelDirs.forEach((dir, i) => {
      if (typeof dir !== 'string') problems.push(`modelDirs[${i}]: not a string`);
      else if (!dir.startsWith('/')) problems.push(`modelDirs[${i}]: "${dir}" — must be an absolute path`);
    });
  }
  ...
  if (!isRecord(portRange)) {
    ...
    if (!isPort(portRange.start)) problems.push('portRange.start: not a port (1024–65535)');
    if (!isPort(portRange.end)) problems.push('portRange.end: not a port (1024–65535)');
    if (isPort(portRange.start) && isPort(portRange.end) && portRange.start > portRange.end) {
  ...
  if (data.engines !== undefined && !isRecord(data.engines)) {
    ...
    for (const [id, engine] of Object.entries(isRecord(data.engines) ? data.engines : {})) {
      if (!isRecord(engine) || typeof engine.binary !== 'string' || engine.binary.length === 0) {
  ...
  if (!isRecord(server)) {
    ...
    if (typeof server.host !== 'string' || server.host.length === 0) problems.push('server.host: not a string');
    if (!isPort(server.port)) problems.push('server.port: not a port (1024–65535)');
  }
  ...
  if (!isRecord(monitoring)) {
    problems.push('monitoring: expected { probeIntervalSec, startupTimeoutSec }');
  } else {
    if (!isFiniteNumber(monitoring.probeIntervalSec) || (monitoring.probeIntervalSec as number) <= 0) {
      problems.push('monitoring.probeIntervalSec: must be a positive number');
    }
    if (!isFiniteNumber(monitoring.startupTimeoutSec) || (monitoring.startupTimeoutSec as number) <= 0) {
 ...
 */
export function validateEngineConfig(data: unknown, engineId = '<engine>'): EngineConfig {
  const problems: string[] = [];
  if (!isRecord(data)) {
    throw new AppError('CONFIG_INVALID', `engines/${engineId}.json: expected a JSON object`, {
      problems: ['not an object'],
    });
  }
  if (typeof data.version !== 'number') problems.push('version: missing or not a number');
  ...
  if (problems.length > 0) {
    throw new AppError('CONFIG_INVALID', `engines/${engineId}.json is invalid`, { problems });
  }
 ...
 */
export function validateModelConfig(data: unknown, modelId = '<model>'): ModelConfig {
  const problems: string[] = [];
  if (!isRecord(data)) {
    throw new AppError('CONFIG_INVALID', `models/${modelId}.json: expected a JSON object`, {
      problems: ['not an object'],
    });
  }
  if (typeof data.version !== 'number') problems.push('version: missing or not a number');
  ...
  if (data.capabilities !== undefined) {
    if (!isRecord(data.capabilities) || Object.values(data.capabilities).some((v) => v !== true && v !== false)) {
...
export function validatePreset(data: unknown, presetName = '<preset>'): Preset {
  const problems: string[] = [];
  if (!isRecord(data)) {
    throw new AppError('CONFIG_INVALID', `preset ${presetName}: expected a JSON object`, {
      problems: ['not an object'],
    });
  }
  if (typeof data.version !== 'number') problems.push('version: missing or not a number');
  if (typeof data.name !== 'string' || data.name.length === 0) problems.push('name: expected a non-empty string');
  if (data.description !== undefined && typeof data.description !== 'string') {
    problems.push('description: not a string');
  }
  if (data.port !== undefined && !isPort(data.port)) problems.push('port: not a port (1024–65535)');
  if (!isRecord(data.params)) problems.push('params: expected an object');

  if (problems.length > 0) {
    throw new AppError('CONFIG_INVALID', `preset ${presetName} is invalid`, { problems });
  }
...
export function mergePartial<T>(current: T, partial: unknown): T {
  if (typeof current !== 'object' || current === null || !isPlainObject(partial)) {
    return { ...current } as T;
  }
  ...
  for (const [key, value] of Object.entries(partial)) {
    ...
    result[key] =
      isPlainObject(currentVal) && isPlainObject(value) ? mergePartial(currentVal, value) : value;
  }
  return result as T;
}
 ...
 */
export function mergeGlobalConfig(current: GlobalConfig, partial: unknown): GlobalConfig {
  if (!isRecord(partial)) {
    throw new AppError('VALIDATION_FAILED', 'body: expected a JSON object with config fields');
  }
