...
describe("validateGlobalConfig (global.json, §9.4)", () => {
  ...
  it("enforces port bounds 1024–65535 (S-9)", () => {
    ...
    expect(
      validateGlobalConfig({ ...base, portRange: { start: 1024, end: 65535 } }),
    ).toBeDefined();
  });
});
...
describe("validateEngineConfig / validateModelConfig / validatePreset", () => {
  it("engine: requires a non-empty binary", () => {
    ...
    expect(() =>
      validateEngineConfig({ version: 1, binary: "", params: {} }),
    ).toThrow(AppError);
    expect(() => validateEngineConfig({ version: 1, params: {} })).toThrow(
      AppError,
    );
  });
  ...
  it("model: params required, capabilities booleans only", () => {
    ...
    expect(() =>
      validateModelConfig({ version: 1, tags: [], capabilities: {} }),
    ).toThrow(AppError);
  });
  ...
  it("model: accepts origin and capabilitiesManual (Faza 3)", () => {
    ...
    expect(() =>
      validateModelConfig({ version: 1, origin: "nope", params: {} }),
    ).toThrow(AppError);
    // capabilitiesManual must be a boolean.
    expect(() =>
      validateModelConfig({
        version: 1,
        capabilitiesManual: "yes",
        params: {},
      }),
    ).toThrow(AppError);
  });
  ...
  it("preset: port bounds + name required", () => {
    expect(
      validatePreset({ version: 1, name: "szybka", port: 8081, params: {} }),
    ).toBeDefined();
    expect(() => validatePreset({ version: 1, name: "", params: {} })).toThrow(
      AppError,
    );
    expect(() =>
      validatePreset({ version: 1, name: "x", port: 99999, params: {} }),
    ).toThrow(AppError);
  });
});
...
describe("mergeGlobalConfig (PUT /api/v1/config/global)", () => {
  ...
  it("merges a partial update over the current config", () => {
    const merged = mergeGlobalConfig(base, {
      server: { port: 3200 },
  ...
  it("validates the merged result", () => {
    expect(() =>
      mergeGlobalConfig(base, { portRange: { start: 99999, end: 8099 } }),
    ).toThrow(AppError);
  });
});
