...
async function tempSseApp(): Promise<SseApp> {
  ...
  const app = await buildApp({ store, getConfigState: () => configState, sse: hub, manager });
 ...
 */
function readSse(url: string, publish: () => void, timeoutMs = 4000): Promise<string> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error(`SSE timeout. url=${url}`)), timeoutMs);
