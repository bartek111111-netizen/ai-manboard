...
describe("preset endpoints (PLAN §14.1, Faza 6.1)", () => {
  it("PUT then GET lists the preset", async () => {
    ...
    const list = await app.inject({
      method: "GET",
      url: "/api/v1/models/m1/presets",
    });
    expect(list.statusCode).toBe(200);
    expect(list.json().presets.map((p: { name: string }) => p.name)).toEqual([
      "fast",
    ]);
    await app.close();
  });
  ...
  it("POST duplicate clones under a new name", async () => {
    ...
    expect(
      list
        .json()
        .presets.map((p: { name: string }) => p.name)
        .sort(),
    ).toEqual(["fast", "fast-copy"]);
    await app.close();
  });
  ...
  it("PUT / DELETE / duplicate for an unknown model → 404", async () => {
    ...
    const put = await app.inject({
      method: "PUT",
      url: "/api/v1/models/nope/presets/x",
      payload: { params: {} },
    });
    ...
    const dup = await app.inject({
      method: "POST",
      url: "/api/v1/models/nope/presets/x/duplicate",
      payload: {},
    });
