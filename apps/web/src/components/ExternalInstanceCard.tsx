    ...
    <div className="status-card status-card--external">
      <div className="status-card-header">
        <div className="status-card-names">
          ...
          {ext.preset && (
            <span className="status-card-preset">{ext.preset}</span>
          )}
        </div>
      ...
      <section className="metric-group">
        ...
        {ext.cmdline.length > 0 ? (
          <CommandBlock
            command={ext.cmdline.join(" ")}
            className="cmd-block--inline"
          />
        ) : (
          <p className="muted">—</p>
        )}
      ...
      <section className="metric-group">
        ...
        {ext.gpu ? (
          <dl className="kv">
            ...
            <dd>
              {ext.gpu.memoryUsedMB !== null &&
              ext.gpu.memoryUsedMB !== undefined &&
              ext.gpu.memoryTotalMB !== null &&
              ext.gpu.memoryTotalMB !== undefined
                ? `${ext.gpu.memoryUsedMB} / ${ext.gpu.memoryTotalMB} MB`
                : "—"}
            </dd>
            <dt>{t("metricsGpuUtilization")}</dt>
            <dd>
              {ext.gpu.utilization !== null && ext.gpu.utilization !== undefined
                ? `${ext.gpu.utilization}%`
                : "—"}
            </dd>
          </dl>
