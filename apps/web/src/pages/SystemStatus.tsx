...
function ProgressBar({ value, max, label }: { value: number; max: number; label: string }) {
  ...
  const color = pct > 90 ? 'var(--color-state-error)' : pct > 70 ? 'var(--color-state-stopping)' : 'var(--color-state-running)';
  return (
...
export function SystemStatus() {
  ...
  return (
    <section className="system-status">
      ...
      {error && (
        <p className="muted">{error} — system metrics unavailable</p>
      )}
        ...
        <div className="system-card">
          ...
          {ram ? (
            <>
              ...
              <dl className="kv">
                <dt>{t('ramUsed')}</dt>
                <dd>{formatMB(ram.usedMB)} / {formatMB(ram.totalMB)}</dd>
              </dl>
            </>
          ) : (
