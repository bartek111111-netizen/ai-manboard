...
import { ensureHome, resolveHome } from '../../apps/server/src/core/config/paths.js';
...
function main(): void {
  ...
  const binary = process.env.E2E_LLAMA_SERVER_BIN ?? '/home/bat/llama.cpp/build/bin/llama-server';
  const modelPath = resolve(repoRoot, process.env.E2E_MODEL_PATH ?? '.e2e-models/SmolLM2-135M-Instruct-Q2_K.gguf');
  ...
  if (!existsSync(binary)) throw new Error(`llama-server binary not found: ${binary}`);
  ...
  store.writePreset(MODEL_ID, PRESET, { version: 1, name: PRESET, port, params: {} });
  ...
  void (async () => {
    ...
    console.log(`[e2e] spawned: state=${lifecycle.getState(INSTANCE)} (probe running)`);
    ...
    if (state !== 'running') {
      ...
      console.error(`[e2e] FAILED: instance did not reach running (state=${state})`);
    ...
    console.log(`[e2e] OK: full lifecycle (start → running → stop) in ${((Date.now() - t0) / 1000).toFixed(1)} s`);
    done(0);
  })().catch((err) => {
