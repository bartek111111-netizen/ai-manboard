...
describe("ConfigStore (Faza 1.1: atomic writes, .bak, validation)", () => {
  ...
  it("rejects a structurally invalid global.json", () => {
    ...
    writeFileSync(
      home.globalFile,
      JSON.stringify({ version: 1, modelDirs: "not-an-array" }),
    );
    expect(() => store.readGlobal()).toThrow(AppError);
  });
  ...
  it("rejects path-traversal ids (S-4: config ids are file names)", () => {
    ...
    expect(() => store.writeModel("../evil", {} as ModelConfig)).toThrow(
      AppError,
    );
    expect(() => store.writePreset("m1", "../../../x", {} as Preset)).toThrow(
      AppError,
    );
  ...
  it("round-trips engine, model and preset files", () => {
    ...
    expect(store.readPreset("llama-3-8b-instruct-9f3a1c2e", "szybka")).toEqual(
      preset,
    );
    expect(store.listPresets("llama-3-8b-instruct-9f3a1c2e")).toEqual([
      "szybka",
    ]);
    expect(
      existsSync(
        join(home.presetsDir, "llama-3-8b-instruct-9f3a1c2e", "szybka.json"),
      ),
    ).toBe(true);
  });
  ...
  it("snapshot() aggregates all layers", () => {
    const { store } = tempHome();
    store.writeEngine("llama-server", {
      version: 1,
      binary: "/x/llama-server",
      params: { threads: 8 },
    });
